...
\begin{frame}
    ...
    opaque predicates, control flow flattening (tabulková interpretace), mrtvý kód, klonování kódu,
    interleaving, změna kódování, změna struktury polí, expanze literálu,
    obfuskace optimalizacemi, úprava symbolických informací, střídání instrukcí,
    inlining, outlining

    rozšíření podmínek, odstranění knihovních volání a programovacích idiomů,
    přidání redundatních argumentů, paralelizace kódu, loop blocking, loop unrolling,
    loop fission

    obfuskace datového typu, falšování symbolických informací

    TODO další věci přímo v jazyce a popsat ten jazyk
    foreach, redo, restart (proč to obfuskuje), 
